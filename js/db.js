    ...
    return new Promise(function(resolve) {
      try {
        ...
        var sampleList = [
          ...
          "NOVA ESPERANÇA", "TODOS OS PAGODES", "BORBOLETA CEGA", "BAIXA ESSA GUARDA"
        ];
        var authenticMap = {};
        ...
        // 1. Encontrar repertórios SAMBA e Malandro e Malandras
        ...
        repReq.onsuccess = function(e) {
          ...
          if (cursor) {
            ...
            if (v.name === 'Malandro e Malandras') {
              malandroRep = v;
              malandroId = String(v.id);
              if (v.user_email !== 'leoogum23@gmail.com') {
                v.user_email = 'leoogum23@gmail.com';
                v.user_id = 'f9e2fcbe-be30-413b-bccc-15f1b701c2d0';
                cursor.update(v);
              }
            }
            cursor.continue();
          } else {
            // Se Malandro e Malandras não existe, cria
            if (!malandroId) {
              malandroId = 'rep-malandros-' + Date.now();
              repStore.put({
                id: malandroId,
            ...
            // 2. Iterar sobre todas as músicas e separar
            var songReq = songStore.openCursor();
            songReq.onsuccess = function(ev) {
              var sCursor = ev.target.result;
              if (sCursor) {
                var sVal = sCursor.value;
                var sTitle = (sVal.title || '').trim().toUpperCase();
                var isAuthenticSamba = authenticMap[sTitle];

                if (String(sVal.repertoireId) === String(sambaId)) {
                  if (!isAuthenticSamba || sVal.user_email === 'leoogum23@gmail.com') {
                    // Mover para Malandro e Malandras
